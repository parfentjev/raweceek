...
const find = {
  elementBy(args: { id: string }): HTMLElement {
    const e = document.getElementById(args.id);
    if (e == null) throw Error(`element not found by id: ${args.id}`);

    return e;
  },
  templateBy(args: { id: string }): HTMLTemplateElement {
    const e = document.getElementById(args.id) as HTMLTemplateElement;
    if (e == null) throw Error(`template not found by id: ${args.id}`);

    return e;
  },
  childBy(args: { parent: ParentNode; selector: string }): HTMLElement {
    const e = args.parent.querySelector(args.selector) as HTMLElement;
    if (e == null) throw Error(`child not found by selector: ${args.selector}`);
...
const clone = (template: HTMLTemplateElement) => {
  ...
  if (child == null) throw Error(`invalid HTML template`);
...
const run = async () => {
  ...
  const parseResponse = (response: Response): Promise<StatusResponse> => {
    if (!response.ok) {
  ...
  await fetch("/api/v2/status")
    ...
    .then((data) => {
      ...
      for (const session of upcoming_sessions) {
        ...
        if (summaryHeader == null || startTimeHeader == null) throw Error(`Missing session data`);
        ...
        for (const countdown of session.countdowns) {
          ...
          if (countdownContainer == null) throw Error();
